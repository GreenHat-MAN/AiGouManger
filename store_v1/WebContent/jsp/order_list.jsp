...
<div class="container">
        ...
        <div style="margin:0 auto; margin-top:10px;width:950px;">
            ...
            <table class="table table-bordered">
                <c:forEach var="order" items="${dto.list}">
                    <tbody>
                    <tr class="success">
                        <th colspan="5">订单编号:${order.oid} 总金额:${order.total}</th>
                    </tr>
                    <tr class="warning">
                        <th>图片</th>
                        <th>商品</th>
    ...
    <div style="text-align: center;">
        <ul class="pagination">
            <c:if test="${dto.showFirstPage}">
                <li>
                    <a href="${pageContext.request.contextPath}/OrderServlet?method=findMyOrder"><<</a>
                </li>
            </c:if>
            <c:if test="${dto.showPrevious}">
                <li>
                    <a href="${pageContext.request.contextPath}/OrderServlet?method=findMyOrder&index=${dto.pageIndex-1}"><</a>
                </li>
            </c:if>

            <c:forEach items="${dto.pageList}" var="i">
                <c:if test="${i==dto.pageIndex}">
                    <li class="active"><a href="#">${i}</a></li>
                </c:if>
                <c:if test="${i!=dto.pageIndex}">
                    <li>
                        <a href="${pageContext.request.contextPath}/OrderServlet?method=findMyOrder&index=${i}">${i}</a>
                    </li>
                </c:if>
            </c:forEach>

            <c:if test="${dto.showNext}">
                <li>
                    <a href="${pageContext.request.contextPath}/OrderServlet?method=findMyOrder&index=${dto.pageIndex+1}">></a>
                </li>
            </c:if>
            <c:if test="${dto.showEndPage}">
                <li>
                    <a href="${pageContext.request.contextPath}/OrderServlet?method=findMyOrder&index=${dto.totalCount}">>></a>
                </li>
            </c:if>
        </ul>
    </div>
